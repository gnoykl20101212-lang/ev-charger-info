...
- `FAQ` — 자주 묻는 질문 검색 + 전체 목록 (첨부 엑셀 기반, `data/faq.js`)
- `정보수정요청` — 영업 직원이 궁금한 용어·현황 변경 요청을 남기는 공간. 담당자가 확인 후 완료 표시
...
## 정보수정요청 공유 설정 (Firebase)

정보수정요청 글을 모든 사람이 공유하려면 무료 Firebase(Firestore)를 연결합니다. 설정이 비어 있으면 자동으로 "이 기기에만 저장(localStorage)" 모드로 동작합니다.
